# Vehicle Data Analysis (Excel Project)

# Description
Analyzed vehicle data including manufacturer, model, mileage, and warranty status.

## Analysis Performed
- Calculated miles per year
- Analyzed relationship between vehicle age and mileage
- Built scatter plot visualization

## Key Insights
- Vehicle age shows a clear relationship with mileage, indicating usage trends over time
- Some newer vehicles exhibit high usage patterns, suggesting heavy utilization

## Tools Used
- Microsoft Excel

## Files Included
- Excel dataset
- Visualizations (charts)

## Visualization

The visualizations for this project are included in this folder.

Please open the image files:
- age-vs-mileage.png
- Each-Drivers-mileage.png

You can also view the Excel file for full analysis:
- vehicle-data-analysis.xlsx

## Business Value
This analysis helps:
- Understand vehicle usage patterns
- Identify high-usage vehicles
- Support better maintenance decisions
